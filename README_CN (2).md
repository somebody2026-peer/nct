...
### v3.1.2 (2026-03-02)
- ✅ PWA-WinClaw 双向通信闭环修复：request_id 统一、消息格式修复、WebSocket 初始化
- ✅ 多设备消息隔离：pendingMessageIds 过滤器，双重保障机制
- ✅ 性能优化：移除频繁流式日志，只保留关键节点（减少 95%+）
- ✅ 离线容错增强：重连后自动恢复离线消息队列
- ✅ 差异化错误提示：区分"未绑定设备"和"设备离线"
- ✅ 添加 reconnected 消息类型处理

### v3.1.1 (2026-02-26)
- ✅ PWA 离线消息队列系统（Phase 2）：数据库持久化、TTL 过期策略、自动恢复
- ✅ 差异化错误提示：区分"未绑定设备"和"设备离线"
- ✅ 监控告警模块：健康检查、积压告警、过期告警（5 分钟周期）
- ✅ PWA Markdown 渲染增强：集成 marked 库，完整语法支持
- ✅ 服务器端优化：数据库初始化、asyncio 导入修复、空值防护
- ✅ BUG 修复：PWA 消息重复显示、marked v17 API 兼容
