...
class BatchedAttentionWorkspace(nn.Module):
    ...
    def forward(
        ...
        first_candidate = batch_candidates[0]
        if self.workspace_query.device != first_candidate.device:
            self.workspace_query = nn.Parameter(self.workspace_query.to(first_candidate.device))
...
class BatchedNCTManager(nn.Module):
    ...
    def forward(self, batch_sensory_data):
        """前向传播（简化版，直接处理输入）
        
        Args:
            batch_sensory_data: 感觉输入字典 {'visual': [B, C, H, W]}
        ...
        if isinstance(batch_sensory_data.get('visual'), torch.Tensor):
            batch_sensory_data['visual'] = batch_sensory_data['visual'].to(device)
        
        return self.process_batch(batch_sensory_data)['representations']
