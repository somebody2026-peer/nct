"""
实验 1: 意识状态监测实验 (增强版 v2)
==========================
Exp-1: Consciousness State Monitoring Experiment (Enhanced v2)

改进内容：
1. ✅ 增加 CNN 对照组（LeNet-5）
2. ✅ 扩大样本量（从 350 增加到 700）
3. ✅ 扩展意识分级（4 级分类：HIGH/MODERATE/LOW/UNCONSCIOUS）
4. ✅ 添加统计显著性检验（p 值、置信区间）
"""
...
class Experiment1Config:
    ...
    n_samples_per_level: int = 300  # 从 100 大幅增加到 300
    version: str = "v3-GPU"  # 版本升级
...
class ConsciousnessMonitor:
    ...
    def process_sample(self, image, noise_level):
        ...
        try:
            ...
            # 综合评分：phi 越高越好，fe 越低越好（调整权重）
            composite_score = (phi_val * 2.0) / (fe_val + 1e-6)
            
            # 4 级分类：HIGH/MODERATE/LOW/UNCONSCIOUS（优化阈值）
...
def main():
    ...
    config = Experiment1Config(
        ...
        n_heads=8,
        n_layers=4,
        n_samples_per_level=300,  # 大幅增加到 300
