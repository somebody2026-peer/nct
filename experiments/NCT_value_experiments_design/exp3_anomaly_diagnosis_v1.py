# -*- coding: utf-8 -*-
"""
Experiment 3: Anomaly Diagnosis (V1 - BaseVersion)
===================================================
Exp-3: Anomaly Diagnosis - Multi-dimensional Anomaly Reasoning

Objective: Validate that NCT can not only detect anomalies but also diagnose their causes

Key Differentiator:
- Traditional methods (Autoencoder, One-Class SVM): Only tell you "it's anomalous"
- NCT: Tells you "WHY it's anomalous" and "HOW to handle it"

Diagnosis Patterns:
┌────────────────────┬─────────────────────────────────────────┐
│ Anomaly Type       │ Expected Pattern                         │
├────────────────────┼─────────────────────────────────────────┤
│ Noise Corruption   │ PE↑↑, Φ↓, Attention dispersed            │
│ Occlusion          │ PE↑, Φ↓, Attention focused on boundary   │
│ Out-of-Distribution│ PE↑↑, Φ↓↓, Salience abnormal             │
│ Adversarial        │ PE normal, Φ↓↓, Attention abnormally focused │
│ Concept Drift      │ PE normal, Φ↓, Neuromodulator abnormal   │
└────────────────────┴─────────────────────────────────────────┘

Version History:
- V1 (2026-03-12): Base version implementing 5 anomaly types and diagnosis logic

Author: NCT Research Team
Date: 2026-03-12
"""
...
class Exp3V1:
    """Anomaly Diagnosis Experiment V1"""
    ...
    def __init__(self, use_gpu=False, pretrained_path=None):
        ...
        self.version = f"v1-{self.model_type}"
        ...
        try:
            ...
            if hasattr(state, 'diagnostics') and state.diagnostics:
                ...
                if 'prediction_error' in diag:
                    metrics['pe'] = diag['prediction_error']
                ...
                if 'workspace' in diag:
                    ...
                    if 'winner_salience' in ws:
                        metrics['salience'] = ws['winner_salience']
    ...
    def diagnose(self, metrics):
        ...
        # Normalize thresholds (these may need tuning)
        PE_HIGH = 0.5
        PE_MEDIUM = 0.3
        PHI_LOW = 0.1
        PHI_VERY_LOW = 0.05
        ENTROPY_HIGH = 1.5  # Dispersed attention
        ENTROPY_LOW = 0.5  # Focused attention
    ...
    def run(self):
        ...
        print("\n" + "="*80)
        print("Experiment 3: Anomaly Diagnosis (V1)")
        print("Objective: Validate NCT can diagnose anomaly causes")
        print("="*80)
        ...
        report = {
            ...
            'conclusions': [
                f"V1: Tested {len(anomaly_types)} anomaly types",
...
def main():
    print("="*80)
    print("Experiment 3 V1: NCT Anomaly Diagnosis")
    print("V1: Base version with 5 anomaly types")
    print("="*80)
    ...
    exp = Exp3V1(use_gpu=False, pretrained_path=pretrained_path)
    report = exp.run()
    
    print("\n" + "="*80)
    print("V1 RESULTS SUMMARY")
    print("="*80)
